...
Heron.data.MapContext = {
  ...
  writeContext: function(mapPanel) {
    ...
    if (treePanel && treePanel.jsonTreeConfig != null) {
      ...
      var tree = "<Extension><" + this.prefix + "treeConfig " + this.xmlns + ">" + jsonTree + "</" + this.prefix + "treeConfig></Extension>";
    ...
    for (var i = 0; i < arrTmsLayers.length; i++) {
      ...
      if (jsonTmsLayers == '') jsonTmsLayers += jsonTms
      else
    ...
    for (i = 0; i < arrImgLayers.length; i++) {
      ...
      if (jsonImgLayers == '') jsonImgLayers += jsonImg
      else
  ...
  initComponent: function() {
    ...
    if (this.zoom > 0) {
      this.on("select", function(combo, record, index) {
        ...
        position.transform(new OpenLayers.Projection("EPSG:3857"), this.map.getProjectionObject());
...
Heron.widgets.search.NominatimSearchCombo = Ext.extend(Ext.form.ComboBox, {
  ...
  tpl: '<tpl for="."><tpl for="address"><div class="x-combo-list-item">{house_number} {road} {postcode} {city} {country}</div></tpl></tpl>',
  displayTpl: '<tpl for="."><tpl for="address">{house_number} {road} {city} {country}</tpl></tpl>',
  lang: 'en',
  charset: 'UTF8',
  hideTrigger: true,
...
Heron.widgets.search.FeaturePanel = Ext.extend(Ext.Panel, {
  ...
  initComponent: function() {
    ...
    this.cardPanels = [];
    if (this.tableGrid) this.cardPanels.push(this.tableGrid);
    if (this.propGrid) this.cardPanels.push(this.propGrid);
    var activeItem = 0;
  ...
  loadFeatures: function(features, featureType) {
    ...
    if (this.displayPanels.length > 0) {
      if (this.displayPanels[0] == 'Table') this.displayGrid();
  ...
  setupStore: function(features) {
    ...
    if ((this.columns[0] != null) && (this.columns[0].id == 'btn_detail')) blnBtnExists = true;
    ...
    if (this.autoConfig && features) {
      ...
      if (autoMaxWidth > 0) {
        for (var key in this.columns) {
          if (columnsWidth[this.columns[key].dataIndex]) this.columns[key].width = columnsWidth[this.columns[key].dataIndex];
        }
      }
    } else {
      for (var c = 0; c < this.columns.length; c++) {
  ...
  updateTbarText: function(type) {
    ...
    if ((type) && (type == 'detail') && (objCount > 0)) this.tbarText.setText(__('Result') + ' ' + (this.propGrid.curRecordNr + 1) + ' ' + __('of') + ' ' + objCount);
    else
...
Heron.widgets.search.FormSearchPanel = Ext.extend(GeoExt.form.FormPanel, {
  ...
  onSearchIssued: function(form, action) {
    ...
    this.timer = setInterval(function() {
      ...
      self.updateStatusPanel(Math.floor(new Date().getTime() / 1000 - startTime) + ' ' + __('Seconds') + ' - ' + Heron.Utils.randArrayElm(self.progressMessages));
    }, 4000);
  },
...
Heron.widgets.search.SpatialSearchPanel = Ext.extend(Ext.Panel, {
  ...
  activateThisControl: function(displaycls) {
	Ext.each(this.drawControl.controls, function (control) {
		if(control.displayClass === displaycls) {
			control.activate();
		} else {
			control.deactivate();
		}
  ...
  onSearchSuccess: function(searchPanel, result) {
	...
	this.activateThisControl('olControlNavigation');
  },
...
Heron.widgets.search.GXP_QueryPanel = Ext.extend(gxp.QueryPanel, {
  ...
  onSearchIssued: function() {
    ...
    this.timer = setInterval(function() {
      ...
      self.updateStatusPanel(Math.floor(new Date().getTime() / 1000 - startTime) + ' ' + __('Seconds') + ' - ' + self.progressMessages[Math.floor(Math.random() * self.progressMessages.length)]);
    }, 4000);
  },
  onSearchComplete: function(searchPanel, result) {
    this.searchButton.enable();
